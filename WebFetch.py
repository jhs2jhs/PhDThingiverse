...
class FetcherIO:
    ...
    def webget(self, req, retries):
        try:
            ans = self.opener.open(req).read()
            return self.web_processor(ans, req)
        except urllib2.HTTPError, what:
            return {}
        except urllib2.URLError, what:
            if retries > 0:
...
def fetchio_single(url, web_processor):
    f = FetcherIO(threads=1, get_retrives=3, web_processor=web_processor)
    f.push(url)
    f.start()

def fetchio_multi(links, threads, web_processor, result_processor):
    f = FetcherIO(threads=threads, get_retrives=3, web_processor=web_processor)
    for url in links:
        f.push(url)
    while f.taskleft():
        url, pagedict = f.pop()
        result_processor(pagedict, url)
...
if __name__ == "__main__":
    ...
    f = FetcherIO(threads=50, get_retrives=3, web_processor=web_read.content_script_derived)
    for url in links:
        f.push(url)
    while f.taskleft():
        url,content = f.pop()
